...
class OraclePool(object):
    ...
    def __init__(self):
        ...
        self._conn = OraclePool.__getConn()
    ...
    def insertOne(self, table, columns=[], values=()):
        """
        @summary: 向数据表插入一条记录
        @param sql:要插入的ＳＱＬ格式
        ...
        try:
            return self._cursor.execute(ins_sql % locals(), values)
    ...
    def multi_insert(self, table, columns=[], values=()):
        ...
        placeholder = ','.join([':%s' % (v) for v in columns])
        ins_sql = 'INSERT INTO %(table)s (%(keys)s) VALUES(%(placeholder)s)'
        return self._cursor.executemany(ins_sql % locals(), values)
